using Distributed,Plots, StatsPlots,JSON, Mustache, Printf
    if nworkers()==1
        addprocs(length(Sys.cpu_info())-1)
    else
        addprocs(length(Sys.cpu_info())-1-nworkers())
    end

@everywhere using Distributions, LinearAlgebra, Random,Distributed
@everywhere using BenchmarkTools,SharedArrays
#SparseArrays, SharedArrays

#######################################################################################
#Recibe un grupo y entrega un vector con los indices de los grupos a los que pertenecen
@everywhere function vecgroup(group)
...
@everywhere function cubic_spline(x, y)
...
@everywhere function cubic_spline_interpolate(x_int, x, y, a = nothing, b = nothing)
    if a == nothing
...
@everywhere function true_positive(t, t_inf, t_sin, t_rec,asin,scalar_asint)
	...
	x_vec = vcat([3*t_inf/4,(3*t_inf + t_sin)/4,(t_inf + 3*t_sin) /4, t_sin,t_sin + (t_rec - t_sin)/(2*28)], [t_sin + (t_rec - t_sin) * i / 28 for i in 7:7:42])
    #x_vec = [-7, -3.5,   0,    7,   14,   21,   28,   35,   42]
	if asin==0
		y_vec = [ 0, 0.75, 0.8, 0.8, 0.78,0.6, 0.35, 0.19, 0.08, 0.00,0.00]
	else
		y_vec = [ 0, 0.75, 0.8, 0.8, 0.78, 0.6, 0.35, 0.19, 0.08, 0.00,0.00]*scalar_asint
	end
	#0.55 era 0.63
...
@everywhere function person_p(t,t_inf,t_sin,t_rec,p_sinto,asin,scalar_asint)
    # t : dia relativo con respecto a la expocision, tb puede ser un vector
    # vectores de valores conocidos en eje x e y
	#asin: Si es asintomatico
	#scalar_asint: Cuanto pondera a la curva de los asintomaticos
	x_vec = vcat([t_inf,(3*t_inf + t_sin)/4,(t_inf + 3*t_sin) /4, t_sin,t_sin + (t_rec - t_sin)/(2*28)], [t_sin + (t_rec - t_sin) * i / 28 for i in 7:7:42])
    #x_vec = [-7, -3.5,   0,    7,   14,   21,   28,   35,   42]
	if asin==0
		y_vec = [ 0, 0.4, 0.8, 0.8, 0.78,0.6, 0.35, 0.19, 0.08, 0.00,0.00]*(p_sinto/0.8)
	else
		y_vec = [ 0, 0.4, 0.8, 0.8, 0.78, 0.6, 0.35, 0.19, 0.08, 0.00,0.00]* (p_sinto/0.8)*scalar_asint
	end
...
#x_vec = vcat([t_inf,(3*t_inf + t_sin) / 4,(t_inf + 3*t_sin) / 4, t_sin], [t_sin + (t_rec - t_sin) * i / 28 for i in 7:7:42])
#x_vec = [-7, -3.5,   0,    7,   14,   21,   28,   35,   42]
#if asin==0
#y_vec = [ 0, 0.55, 0.8, 0.79, 0.64, 0.43, 0.19, 0.08, 0.00,0.00]
...
@everywhere function exponential_p(t,t_inf,t_sin,t_rec,p_sinto,t_peak,horas_turno,asin,scalar_asint)
	prob=person_p(t,t_inf,t_sin,t_rec,p_sinto,asin,scalar_asint) #Se calcula el peak que se alcanza en t_peak
	lambda=(log(max(1-prob,0.00000001)))/t_peak #Se calcula el lambda tal que P(tiempo para contagiarse<=t_peak)=probabilidad peak de contagio
...
@everywhere function group_p(M_cont)
...
@everywhere function interaciones(Mrel,v,Rep)
largo=length(v)
Mint=zeros(largo,largo,Rep)
    for i in 1:largo
         for j in i+1:largo
			Mint[i,j,:]=Int.((rand(Rep).<=Mrel[v[i],v[j]]))
            Mint[j,i,:]=Mint[i,j,:]
...
@everywhere function pool(array,num)
	if length(array)==0
		return []
	else
		pools=[]
...
@everywhere function simulation(N,Group,T,Tra,MRELR, f,G,peak,t_peak,p_int,p_ext,γ,p_false_positive,R,Politica,tpool,random,quienes,cuarentena,z,dias_atras,scalar_asint,test_sym,distribuir,v)
...
#MRELR: Matriz de presencia de todos los trabajadores, tiene una tercer componente temporal
...
#v=vecgroup(Group) Cuando se utiliza la cuarentena, con este vector se recuperan los integrantes del grupo para mandarlos a cuarentena
...
#        @sync for pondera in 1:5
#        @async for rep = Int(R/5)*(pondera-1)+1:Int(R/5)*pondera
#@distributed for rep in 1:R
for rep=1:R
        Mint=MRELR[:,:,rep] # Matriz de interacciones para cada repetición
...
########################################################################################
			for i in findall((g.*inf.*(-qu.+1).*(-re.+1)).==1)#Se recorren los abuelitos enfermos, no en cuarentena y no recuperados
				casos=shuffle(Mint[:,i].*su.*(-qu.+1).*collect(1:N))
				...
				for j in  casos2 #Se recorren los abuelitos que nunca se han contagiado y no en cuarentena
						...
						if ((MD[i,t]*MD[j,t])==1) & ((MN[i,t]*MN[j,t])==0)  #Condicion esten ambos trabajando
							pond=1
						end
						if ((MD[i,t]*MD[j,t])==0) & ((MN[i,t]*MN[j,t])==1)
							pond=1
						end
						if ((MD[i,t]*MD[j,t])==1) & ((MN[i,t]*MN[j,t])==1)
							pond=2
						end
						MC[i,j]=exponential_p(t-tEx[i],tinf,tsin,trec,peak[i],t_peak[i],pond*hr,As[i],scalar_asint)
				end
				end
			end
#################################################################################################################
...
Ninterna=(-mm.*p_int.+1)
NPG=(-PG.+1)
Nexterna=(-(-mm.+1).*p_ext.+1)
new_inf+=(-qu.+1).*su.*(r.<(-NPG.*Ninterna.*Nexterna.+1)) #Vector de infectados del grupo que no esten en cuarentena, esten o no trabajando
...
#Se realiza el proceso de cambio en los estados de los nuevos infectados
           	for j in findall(new_inf.==1)                   # cicle over newly infected
				i=j[1]
					...
					Qu[i, Int(min(T,t+t_inc+7)): Int(min(T,max(t+20,t+t_rec+8)))] .= (1-As[i]) #you must go to quarantine if you get symptoms
		...
		if (t>=3)&((cuarentena=="Mixto")|(cuarentena=="Grupal"))              #Si se activa se manda a todo el grupo cuando alguien tiende sintomas
			for i in findall(Sy[:,t].*(Qu[:,t]).*Sy[:,t-1].*(-Qu[:,t-1].+1).*Sy[:,t-2].*(-Qu[:,t-2].+1).==1) #Condicion de no ser pillado hasta los sintomas
				Aux=MD[Int(i),Int(max(t-dias_atras,1)):t]'.*Mint[:,Int(i)]
				...
				Aux2=MN[i,Int(max(t-dias_atras,1)):t]'.*Mint[:,i]
...
###################################con estos numeros se saca una muestra de minimo entre tamaño G y los que trabajan al azar. De momento esta descontinuado
		if test_sym=="Si"
			for i in findall(sy.*(-qu.+1).==1)
									...
									p_positive = true_positive(t-tEx[i], tIn[i]-tEx[i], tSy[i]-tEx[i], tRe[i]-tEx[i],As[i],scalar_asint)
					...
					if length(vector)>=1
						for cand in vector
							...
							for rpi in findall(rp.==0)
								...
								if (qu[rpi]==0)&(rand()<p_false_positive)&(rpi in cand)
									rp[rpi]=1
								end
							end
							suma=0
					...
					if (Politica=="Individual")	& (length(cand)>=1)	#Si al menos hay 1 para testear y no pool
						for i in cand	#se recorren
							...
							NTest[t]+=1   #Un test nuevo
										...
										if (qu[i]==0)& (rand() < p_false_positive) #Caso falso positivo
											Qu[i,min(T,t+1):min(T,t+15)] .= 1 # if someone tests (false) positive, quarantined for 2 weeks
											if cuarentena=="Grupal"
												Aux=MD[Int(i),Int(max(t-dias_atras,1)):t]'.*Mint[:,Int(i)]
													for te in Int(max(t-dias_atras,1)):t
...
#	Infect2[rep]=maximum(sum(In[1:N,:].*(-Qu[1:N,:].+1),dims=1))
    end
...
#MFocos=zeros(R,5)
#for i in 1:5
#MFocos[:,i]=Infect.>=ones(R)*0.1*i*N
#end
#Focos=sum(MFocos,dims=1)/R
#MFocos2=zeros(R,5)
#for i in 1:5
#MFocos2[:,i]=Infect2.>=ones(R)*0.1*i*N
#end
#Focos2=sum(MFocos2,dims=1)/R
    return mQua,mQua2,mInf,mInf2,mInf3, mNFpositive,  mNTest,mNTest2, mSy, VInf
end
################################################################################################################################################
@everywhere function leer(dict12)
	...
	Pasint=dict12["Porcentaje asintomaticos"]
	Porasint=(rand(N,Repeticiones).<=Pasint)
...
return N, Group,T,Horario,Mrel, Diastest,Testgrupo,p_int,p_ext,Porasint,Probfp,Repeticiones,Politica,tpool,test,quienes,Cuaren,Diascuarentena,Diasatras,TestSyn,distribuir,NombreGrupo,NombreHorario, NameP, infopar, Pasint
...
function graficar(arrays,Group,ggrafico,nombreg,NombresP,titulo,x,y,T)
	...
	for a in 1:length(arrays)
		...
		if (titulo=="Tests Totales") | (titulo=="Tests Sintomáticos") | (titulo=="Tests Aleatorios")
			if a==1
				grafico=scatter(1:T,arr,label=NombresP[a],lw=3,markerstrokecolor=:false)
			else
				scatter!(1:T,arr,label=NombresP[a],lw=3,markerstrokecolor=:false)
			end
		else
			if a==1
...
if ff==1
#Colocar el numero de procesadores deacuerdo al pc
#procesadores=9
        global procesadores
#Paralelizar las simulaciones usando 9 procesadores
global graficogrupo,nombregrafico,t_peak,scalar_asint,peak
fs=Array{Any}(UndefInitializer(),procesadores)
global fs
        fs2=Array{Any}(UndefInitializer(),procesadores)
global fs2
        fs3=Array{Any}(UndefInitializer(),procesadores)
global fs3
        fs4=Array{Any}(UndefInitializer(),procesadores)
global fs4

global N,Group,T,Horario,Mrel,Diastest,Testgrupo,p_int,p_ext,Porasint,Probfp,Repeticiones,Politica,tpool,test,quienes,Cuaren,Diascuarentena,Diasatras,TestSyn,distribuir,NombreGrupo,NombreHorario,NameP,infopar,Pasint
N,Group,T,Horario,Mrel,Diastest,Testgrupo,p_int,p_ext,Porasint,Probfp,Repeticiones,Politica,tpool,test,quienes,Cuaren,Diascuarentena,Diasatras,TestSyn,distribuir,NombreGrupo,NombreHorario,NameP,infopar,Pasint=leer(dict12)
t_peak=ones(N)*18                 #Tiempo peak de contagio
...
nombregrafico=[["Total"]' [i for i in nomb]']'
 ...
 rr=Int(floor(Repeticiones/procesadores))
 global rr
vg=vecgroup(Group)
global vg
MRELR=interaciones(Mrel,vg,rr*procesadores)
Repeticiones22=rr*procesadores
global Repeticiones22
global MRELR

global ff, base_mQua,base_mQua2, base_mInf,base_mInf2,base_mInf3, base_mNFp, base_T, base_T2,base_mSy, base_VInf

global qwer1,qwer2,qwer3,qwer4,qwer5,qwer6,qwer7,qwer8,qwer9,qwer10
    qwer1 = zeros(N,T,procesadores,4)
    qwer2 = zeros(N,T,procesadores,4)
    qwer3 = zeros(N,T,procesadores,4)
    qwer4 = zeros(N,T,procesadores,4)
    qwer5 = zeros(N,T,procesadores,4)
    qwer6= zeros(T,procesadores,4)
	qwer7=zeros(T,procesadores,4)
	qwer8=zeros(T,procesadores,4)
	qwer9=zeros(N,T,procesadores,4)
    qwer10=zeros(N,T,procesadores,4)

for iz in 1:procesadores
          fs[iz] = @spawn simulation(N,Group,T,Horario,MRELR[:,:,rr*(iz-1)+1:rr*iz], Diastest,Testgrupo,peak,t_peak,p_int,p_ext,Porasint,Probfp,rr,Politica,tpool,test,quienes,Cuaren,Diascuarentena,Diasatras,scalar_asint,TestSyn,distribuir,vg)
           end
      for iz in 1:procesadores
  qwer1[:,:,iz,1],qwer2[:,:,iz,1],qwer3[:,:,iz,1],qwer4[:,:,iz,1],qwer5[:,:,iz,1],qwer6[:,iz,1],qwer7[:,iz,1],qwer8[:,iz,1],qwer9[:,:,iz,1],qwer10[:,:,iz,1]=fetch(fs[iz])

end
        base_mQua=sum(qwer1[:,:,:,1],dims=3)/Repeticiones22
        base_mQua2=sum(qwer2[:,:,:,1],dims=3)/Repeticiones22
        base_mInf=sum(qwer3[:,:,:,1],dims=3)/Repeticiones22
        base_mInf2=sum(qwer4[:,:,:,1],dims=3)/Repeticiones22
        base_mInf3=sum(qwer5[:,:,:,1],dims=3)/Repeticiones22
        base_mNFp=sum(qwer6[:,:,1],dims=2)/Repeticiones22
        base_T=sum(qwer7[:,:,1],dims=2)/Repeticiones22
        base_T2=sum(qwer8[:,:,1],dims=2)/Repeticiones22
        base_mSy=sum(qwer9[:,:,:,1],dims=3)/Repeticiones22
        base_VInf=sum(qwer10[:,:,:,1],dims=3)/Repeticiones22
end
if ff==2
 global N2,Group2,T2,Horario2,Mrel2, Diastest2,Testgrupo2,p_int2,p_ext2,Porasint2,Probfp2,Repeticiones2,Politica2,tpool2,test2,quienes2,Cuaren2,Diascuarentena2,Diasatras2,TestSyn2,distribuir2,NombreGrupo2,NombreHorario2,NameP2,infopar2, Pasint2
 N2,Group2,T2,Horario2,Mrel2, Diastest2,Testgrupo2,p_int2,p_ext2,Porasint2,Probfp2,Repeticiones2,Politica2,tpool2,test2,quienes2,Cuaren2,Diascuarentena2,Diasatras2,TestSyn2,distribuir2,NombreGrupo2,NombreHorario2,NameP2,infopar2,Pasint2=leer(dict12)
global ideal_mQua,ideal_mQua2, ideal_mInf,ideal_mInf2,ideal_mInf3, ideal_mNFp, ideal_T,ideal_T2, ideal_mSy, ideal_VInf

for iz in 1:procesadores
         fs2[iz]= @spawn  simulation(N,Group,T,Horario,MRELR[:,:,rr*(iz-1)+1:rr*iz],Diastest2,Testgrupo2,peak,t_peak,p_int,p_ext,Porasint,Probfp,rr,Politica2,tpool2,test2,quienes2,Cuaren2,Diascuarentena2,Diasatras2,scalar_asint,TestSyn2,distribuir2,vg)
    end
      for iz in 1:procesadores
qwer1[:,:,iz,2],qwer2[:,:,iz,2],qwer3[:,:,iz,2],qwer4[:,:,iz,2],qwer5[:,:,iz,2],qwer6[:,iz,2],qwer7[:,iz,2],qwer8[:,iz,2],qwer9[:,:,iz,2],qwer10[:,:,iz,2]=fetch(fs2[iz])
end
        ideal_mQua=sum(qwer1[:,:,:,2],dims=3)/Repeticiones22
        ideal_mQua2=sum(qwer2[:,:,:,2],dims=3)/Repeticiones22
        ideal_mInf=sum(qwer3[:,:,:,2],dims=3)/Repeticiones22
        ideal_mInf2=sum(qwer4[:,:,:,2],dims=3)/Repeticiones22
        ideal_mInf3=sum(qwer5[:,:,:,2],dims=3)/Repeticiones22
        ideal_mNFp=sum(qwer6[:,:,2],dims=2)/Repeticiones22
        ideal_T=sum(qwer7[:,:,2],dims=2)/Repeticiones22
        ideal_T2=sum(qwer8[:,:,2],dims=2)/Repeticiones22
        ideal_mSy=sum(qwer9[:,:,:,2],dims=3)/Repeticiones22
        ideal_VInf=sum(qwer10[:,:,:,2],dims=3)/Repeticiones22
end
    if ff==3
 global N3,Group3,T3,Horario3,Mrel3, Diastest3,Testgrupo3,p_int3,p_ext3,Porasint3,Probfp3,Repeticiones3,Politica3,tpool3,test3,quienes3,Cuaren3,Diascuarentena3,Diasatras3,TestSyn3,distribuir3,NombreGrupo3,NombreHorario3,NameP3,infopar3, Pasint3
 N3,Group3,T3,Horario3,Mrel3, Diastest3,Testgrupo3,p_int3,p_ext3,Porasint3,Probfp3,Repeticiones3,Politica3,tpool3,test3,quienes3,Cuaren3,Diascuarentena3,Diasatras3,TestSyn3,distribuir3,NombreGrupo3,NombreHorario3,NameP3,infopar3, Pasint3=leer(dict12)
global f1_mQua,f1_mQua2, f1_mInf,f1_mInf2,f1_mInf3, f1_mNFp, f1_T, f1_T2, f1_mSy,f1_VInf

for iz in 1:procesadores
fs3[iz]= @spawn  simulation(N,Group,T,Horario,MRELR[:,:,rr*(iz-1)+1:rr*iz], Diastest3,Testgrupo3,peak,t_peak,p_int,p_ext,Porasint,Probfp,rr,Politica3,tpool3,test3,quienes3,Cuaren3,Diascuarentena3,Diasatras3,scalar_asint,TestSyn3,distribuir3,vg)
    end
for iz in 1:procesadores
 qwer1[:,:,iz,3],qwer2[:,:,iz,3],qwer3[:,:,iz,3],qwer4[:,:,iz,3],qwer5[:,:,iz,3],qwer6[:,iz,3],qwer7[:,iz,3],qwer8[:,iz,3],qwer9[:,:,iz,3],qwer10[:,:,iz,3]=fetch(fs3[iz])
end
        f1_mQua=sum(qwer1[:,:,:,3],dims=3)/Repeticiones22
        f1_mQua2=sum(qwer2[:,:,:,3],dims=3)/Repeticiones22
        f1_mInf=sum(qwer3[:,:,:,3],dims=3)/Repeticiones22
        f1_mInf2=sum(qwer4[:,:,:,3],dims=3)/Repeticiones22
        f1_mInf3=sum(qwer5[:,:,:,3],dims=3)/Repeticiones22
        f1_mNFp=sum(qwer6[:,:,3],dims=2)/Repeticiones22
        f1_T=sum(qwer7[:,:,3],dims=2)/Repeticiones22
        f1_T2=sum(qwer8[:,:,3],dims=2)/Repeticiones22
        f1_mSy=sum(qwer9[:,:,:,3],dims=3)/Repeticiones22
        f1_VInf=sum(qwer10[:,:,:,3],dims=3)/Repeticiones22
end
if ff==4
 global N4,Group4,T4,Horario4,Mrel4, Diastest4,Testgrupo4,p_int4,p_ext4,Porasint4,Probfp4,Repeticiones4,Politica4,tpool4,test4,quienes4,Cuaren4,Diascuarentena4,Diasatras4,TestSyn4,distribuir4,graficogrupo4,nombregrafico4,NombreGrupo4,NombreHorario4,NameP4,infopar4, Pasint4
 N4,Group4,T4,Horario4,Mrel4, Diastest4,Testgrupo4,p_int4,p_ext4,Porasint4,Probfp4,Repeticiones4,Politica4,tpool4,test4,quienes4,Cuaren4,Diascuarentena4,Diasatras4,TestSyn4,distribuir4,NombreGrupo4,NombreHorario4,NameP4,infopar4,Pasint4=leer(dict12)
global fr_mQua,fr_mQua2, fr_mInf,fr_mInf2,fr_mInf3, fr_mNFp, fr_T, fr_T2,fr_mSy, fr_VInf

for iz in 1:procesadores
fs4[iz]= @spawn  simulation(N,Group,T,Horario,MRELR[:,:,rr*(iz-1)+1:rr*iz], Diastest4,Testgrupo4,peak,t_peak,p_int,p_ext,Porasint,Probfp,rr,Politica4,tpool4,test4,quienes4,Cuaren4,Diascuarentena4,Diasatras4,scalar_asint,TestSyn4,distribuir4,vg)
    end
    for iz in 1:procesadores
qwer1[:,:,iz,4],qwer2[:,:,iz,4],qwer3[:,:,iz,4],qwer4[:,:,iz,4],qwer5[:,:,iz,4],qwer6[:,iz,4],qwer7[:,iz,4],qwer8[:,iz,4],qwer9[:,:,iz,4],qwer10[:,:,iz,4]=fetch(fs4[iz])
end
        fr_mQua=sum(qwer1[:,:,:,4],dims=3)/Repeticiones22
        fr_mQua2=sum(qwer2[:,:,:,4],dims=3)/Repeticiones22
        fr_mInf=sum(qwer3[:,:,:,4],dims=3)/Repeticiones22
        fr_mInf2=sum(qwer4[:,:,:,4],dims=3)/Repeticiones22
        fr_mInf3=sum(qwer5[:,:,:,4],dims=3)/Repeticiones22
        fr_mNFp=sum(qwer6[:,:,4],dims=2)/Repeticiones22
        fr_T=sum(qwer7[:,:,4],dims=2)/Repeticiones22
        fr_T2=sum(qwer8[:,:,4],dims=2)/Repeticiones22
        fr_mSy=sum(qwer9[:,:,:,4],dims=3)/Repeticiones22
        fr_VInf=sum(qwer10[:,:,:,4],dims=3)/Repeticiones22
end
ff+=1
...
if ff==3
	...
	QuaP=[sum(base_mQua2),sum(ideal_mQua2)]
...
#FocosP=[base_Focos,ideal_Focos]
#FocosP2=[base_Focos2,ideal_Focos2]
end
if ff==4
	...
	QuaP=[sum(base_mQua2),sum(ideal_mQua2),sum(f1_mQua2)]
...
#FocosP=[base_Focos,ideal_Focos,f1_Focos]
#FocosP2=[base_Focos2,ideal_Focos2,f1_Focos2]
end
if ff==5
	...
	QuaP=[sum(base_mQua2),sum(ideal_mQua2),sum(f1_mQua2),sum(fr_mQua2)]
...
#FocosP=[base_Focos,ideal_Focos,f1_Focos,fr_Focos]
#FocosP2=[base_Focos2,ideal_Focos2,f1_Focos2,fr_Focos2]
...
for g in ggrafico
    ...
    if length(NombresP)==1
g=plot([i-1 for i in 2:ff]',arrays',seriestype=:bar,label=NombresP[1],xlims=(0.4,4.6),ylims=(0,maximum( [10,maximum(arrays)]  ) ))
    else
...
if ff==2
	arrays=[[base_T.+base_T2]]
end
if ff==3
	arrays=[[base_T.+base_T2],[ideal_T.+ideal_T2]]
end
if ff==4
	arrays=[[base_T.+base_T2],[ideal_T.+ideal_T2],[f1_T.+f1_T2]]
end
if ff==5
	arrays=[[base_T.+base_T2],[ideal_T.+ideal_T2],[f1_T.+f1_T2],[fr_T.+fr_T2]]
end
...
if ff==2
	arrays=[sum(base_T.+base_T2)]
end
if ff==3
	arrays=[sum(base_T.+base_T2),sum(ideal_T.+ideal_T2)]
end
if ff==4
	arrays=[sum(base_T.+base_T2),sum(ideal_T.+ideal_T2),sum(f1_T.+f1_T2)]
end
if ff==5
	arrays=[sum(base_T.+base_T2),sum(ideal_T.+ideal_T2),sum(f1_T.+f1_T2),sum(fr_T.+fr_T2)]
end
...
\end{tabular}
\caption{Tabla resumen \textit{infectados activos} para la \ifnum {{#:AB}}{{NumP}}{{/:AB}}>1 {comparación de las {{#:AB}}{{NumP}}{{/:AB}} políticas}\else {política}\fi, con un total de  {{#:AB}}{{N}}{{/:AB}} personas. }
\label{tab:ResumenInfectadosActivos}
\end{table}

\newpage
\subsection{Infectados Activos No Detectados Trabajando}
Los \textit{infectados activos no detectados trabajando} corresponden a aquellos individuos que además de estar con el virus activo, no han sido detectados y están en turno de trabajo. \ifnum {{#:AB}}{{NumP}}{{/:AB}}>1 {La Figura~\ref{fig:InfectadosTrabajando} muestra los \textit{infectados activos no detectados trabajando} promedio (de entre todas las simulaciones) para cada día con cada una de las políticas de testeo. Se puede apreciar que la política que tiene el peak más alto de infectados es la{{#:AB}}\textbf{ {{MInfectadosTpeakP}} }{{/:AB}}alcanzando un total de {{#:AB}}{{MInfectadosTpeak}}{{/:AB}} infectados en el día {{#:AB}}{{MInfectadosTpeakD}}{{/:AB}}. Por otro lado la política con el menor peak es la de{{#:AB}}\textbf{ {{mInfectadosTpeakP}} }{{/:AB}}con un peak de {{#:AB}}{{mInfectadosTpeak}}{{/:AB}} infectados activos en el día {{#:AB}}{{mInfectadosTpeakD}}{{/:AB}}.} \else{La Figura~\ref{fig:InfectadosTrabajando} muestra los \textit{infectados activos no detectados trabajando} promedio (de entre todas las simulaciones) para cada día. Se puede apreciar que la política{{#:AB}}\textbf{ {{MInfectadosTpeakP}} }{{/:AB}}alcanzando un total de {{#:AB}}{{MInfectadosTpeak}}{{/:AB}} infectados en el día {{#:AB}}{{MInfectadosTpeakD}}{{/:AB}}.} \fi
...
\centering
	...
	\caption{Gráfico de los \textit{infectados activos no detectados trabajando} \ifnum {{#:AB}}{{NumP}}{{/:AB}}>1 {comparando las   {{#:AB}}{{NumP}}{{/:AB}} políticas}\else {para la política}\fi, con un total de  {{#:AB}}{{N}}{{/:AB}} personas.  }
...
\end{tabular}
\caption{Tabla resumen \textit{infectados acumulados} para la \ifnum {{#:AB}}{{NumP}}{{/:AB}}>1 {comparación de las {{#:AB}}{{NumP}}{{/:AB}} políticas}\else {política}\fi, con un total de  {{#:AB}}{{N}}{{/:AB}} personas. }
...
\ifnum {{#:AB}}{{NumP}}{{/:AB}}>1 {A continuación se detalla la cantidad de \textit{tests utilizados} por cada una de las políticas estudiadas.{{#:AB}}\textbf{ {{MTestTotalP}} }{{/:AB}}es la política que utiliza un mayor número tests, utilizando {{#:AB}}{{MTestTotal}}{{/:AB}} tests. Por el contrario la política{{#:AB}}\textbf{ {{mTestTotalP}} }{{/:AB}}es la que utiliza menos tests ocupando {{#:AB}}{{mTestTotal}}{{/:AB}} tests. En la Figura~\ref{fig:Tests} se puede observar número de \textit{tests utilizados} para cada día.} \else { A continuación en la Figura~\ref{fig:Tests} se detalla el número de \textit{tests diarios utilizados} por la política{{#:AB}}\textbf{ {{MTestTotalP}} }{{/:AB}}la cual ocupó {{#:AB}}{{MTestTotal}}{{/:AB}} tests.} \fi
\begin{figure}[H]
\centering
    \includegraphics[scale=0.45]{Guardar/{{#:AB}}{{Caso}}{{/:AB}}/Tests1.pdf}
	\caption{ \ifnum {{#:AB}}{{NumP}}{{/:AB}}>1 {Gráfico de los \textit{test diarios utilizados} al comparar las   {{#:AB}}{{NumP}}{{/:AB}} políticas, con un total de  {{#:AB}}{{N}}{{/:AB}} personas.  }\else {Gráfico de los \textit{test diarios utilizados}, con un total de  {{#:AB}}{{N}}{{/:AB}} personas. }\fi}
	\label{fig:Tests}
\end{figure}
\ifnum {{#:AB}}{{NumP}}{{/:AB}}>1 {
En la Figura~\ref{fig:Testshistograma} se observa el número de \textit{tests utilizados} por las  {{#:AB}}{{NumP}}{{/:AB}} políticas.
\begin{figure}[H]
\centering
...
Los detalles con el número de tests utilizados en sintomáticos y testeos aleatorios, se encuentra en las Sección~\ref{subsec:TestsAleatorios} y \ref{subsec:TestsSintomaticos} respectivamente.
...
\end{tabular}
\caption{Tabla resumen \textit{personas en cuarentena} para la \ifnum {{#:AB}}{{NumP}}{{/:AB}}>1 {comparación de las {{#:AB}}{{NumP}}{{/:AB}} políticas}\else {política}\fi, con un total de  {{#:AB}}{{N}}{{/:AB}} personas. }
...
{{#:A2}}\begin{figure}[H]
\centering
  \includegraphics[scale=0.4]{Guardar/{{#:AB}}{{Caso}}{{/:AB}}/Infectados{{Ind}}.pdf}
...
{{#:A2}} \begin{figure}[H]
\centering
  \includegraphics[scale=0.4]{Guardar/{{#:AB}}{{Caso}}{{/:AB}}/InfectadosTrabajando{{Ind}}.pdf}
...
{{#:A2}} \begin{figure}[H]
\centering
  \includegraphics[scale=0.4]{Guardar/{{#:AB}}{{Caso}}{{/:AB}}/InfectadosAcumulado{{Ind}}.pdf}
...
\begin{figure}[H]
\centering
    \includegraphics[scale=0.4]{Guardar/{{#:AB}}{{Caso}}{{/:AB}}/TestsAleatorios1.pdf}
...
\begin{figure}[H]
\centering
    \includegraphics[scale=0.4]{Guardar/{{#:AB}}{{Caso}}{{/:AB}}/TestsAleatoriosHistograma1.pdf}
	\caption{Histograma de los \textit{test totales utilizados} \ifnum {{#:AB}}{{NumP}}{{/:AB}}>1 {al comparar las   {{#:AB}}{{NumP}}{{/:AB}} políticas}\else{por la política}\fi, con un total de  {{#:AB}}{{N}}{{/:AB}} personas.  }
...
\begin{figure}[H]
\centering
    \includegraphics[scale=0.4]{Guardar/{{#:AB}}{{Caso}}{{/:AB}}/TestsSintomaticos1.pdf}
	\caption{Gráfico de los \textit{test diarios} utilizados \ifnum {{#:AB}}{{NumP}}{{/:AB}}>1 {al comparar las   {{#:AB}}{{NumP}}{{/:AB}} políticas}\else{por la política}\fi, con un total de  {{#:AB}}{{N}}{{/:AB}} personas.  }
...
\begin{figure}[H]
\centering
    \includegraphics[scale=0.4]{Guardar/{{#:AB}}{{Caso}}{{/:AB}}/TestsSintomaticosHistograma1.pdf}
	\caption{Histograma de los \textit{test totales} utilizados \ifnum {{#:AB}}{{NumP}}{{/:AB}}>1 {al comparar las {{#:AB}}{{NumP}}{{/:AB}} políticas}\else{por la política}\fi, con un total de  {{#:AB}}{{N}}{{/:AB}} personas.  }
...
{{#:A2}} \begin{figure}[H]
\centering
  \includegraphics[scale=0.4]{Guardar/{{#:AB}}{{Caso}}{{/:AB}}/Cuarentena{{Ind}}.pdf}
